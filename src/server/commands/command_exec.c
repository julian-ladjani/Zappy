...
{
	list_t *team_list = cmdparams->teams;
	server_team_t *team;
	...
	while (team_list) {
		...
		if (team && !strcmp(team->name, cmdparams->name)) {
			user->logged_type = ZAPPY_USER_CONNECTED;
				...
				server_user_t *user)
{
	...
	if (user->logged_state & ZAPPY_USER_NOT_CONNECTED)
		...
		if (user->type == ZAPPY_USER_AI)
			search_ai_command(server_config, user, cmdparams);
		if (user->type == ZAPPY_USER_GRAPHIC)
			search_graphic_command(server_config, user, cmdparams);
	}
